...
BOT_TOKEN = "7904843747:AAFSb1TkUDEKSTJfsxbD3on4QhadZpOaPTM"
OWNER_ID = 6435499094
...
import html  # Standard Python library, no import errors possible
from telegram.constants import ParseMode # The modern way to handle parse modes
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        text = "<b>🌍 Global Leaderboard (Top 25)</b>\n\n"
        for i, r in enumerate(rows, 1):
            # r[0] is now the 'display_name' we created in SQL
            name = html.escape(str(r[0]))
            text += f"#{i} {name} - 🔥 {r[3]} pts!\n"
            text += f"✅ Correct: {r[2]} ✖️ Incorrect: {r[1]-r[2]}\n\n"
...
async def nightly_leaderboard_job(context: ContextTypes.DEFAULT_TYPE):
    """Sends both Global and Group performance reports to all groups at night."""
    
    # 1. Get Global Top 10 Data once (to save resources)
    global_rows = db.get_leaderboard_data(limit=10)
    global_text = "<b>🌍 Global Top 10 Performers</b>\n"
    ...
    if not global_rows:
        ...
        for i, r in enumerate(global_rows, 1):
            name = html.escape(str(r[0]))
            global_text += f"#{i} {name} - ⭐ {r[3]} pts\n"

    # 2. Get list of all groups
    with db.get_db() as conn:
        chats = conn.execute("SELECT chat_id, title FROM chats WHERE type != 'private'").fetchall()

    # 3. Iterate through groups and send custom report
    for c in chats:
        chat_id = c[0]
        chat_title = html.escape(c[1] if c[1] else "This Group")
        
        try:
            # Get Group-specific data
            group_rows = db.get_leaderboard_data(chat_id=chat_id, limit=10)
            group_text = f"\n<b>👥 {chat_title} Top 10</b>\n"
            ...
            if not group_rows:
                ...
                for i, r in enumerate(group_rows, 1):
                    name = html.escape(str(r[0]))
                    group_text += f"#{i} {name} - ⭐ {r[3]} pts\n"

            # Combine everything into one message
            final_message = (
                "🌙 <b>DAILY COMPETITION SUMMARY</b>\n"
                "━━━━━━━━━━━━━━━━━━\n"
                f"{global_text}\n"
                "━━━━━━━━━━━━━━━━━━\n"
                f"{group_text}\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "<i>Great job today everyone! See you tomorrow for more quizzes!</i>"
            )
            ...
            # Small delay to avoid hitting Telegram broadcast limits
            await asyncio.sleep(0.5) 
            
        except Exception as e:
            logger.error(f"Could not send nightly report to {chat_id}: {e}")
            continue
