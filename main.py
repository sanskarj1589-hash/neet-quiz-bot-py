...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Processes the user's answer and updates streaks/scores with group-specific compliments."""
    ...
    if comp:
        ...
        final_text = compliment_text.replace("{user}", f"*{answer.user.first_name}*")

        if chat_id < 0: # Send in groups
            try:
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Displays the Global Leaderboard (Top 25)."""
...
async def groupleaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Displays the Leaderboard specific to the current group."""
...
if __name__ == '__main__':
    ...
    # This ensures all tables (group_settings, etc.) exist before the bot tries to use them.
    db.init_db()
    ...
    # We use a try-except here to prevent the bot from crashing if the 'settings' table is fresh
    try:
        with db.get_db() as conn:
            row = conn.execute("SELECT value FROM settings WHERE key='autoquiz_interval'").fetchone()
            interval_min = int(row[0]) if row else 30
    except Exception:
        interval_min = 30 # Default fallback
        
    jq.run_repeating(auto_quiz_job, interval=interval_min * 60, first=20)
    jq.run_daily(nightly_leaderboard_job, time=time(hour=21, minute=0)) # 9 PM
