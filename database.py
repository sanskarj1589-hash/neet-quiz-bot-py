...
def update_user_stats(user_id, chat_id, is_correct, username=None, first_name=None):
    ...
    with get_db() as conn:
        ...
        # Accessing ResultSet by row index for Libsql compatibility
...
def get_compliment(c_type):
    with get_db() as conn:
        res_settings = conn.execute("SELECT value FROM settings WHERE key='compliments_enabled'")
        if not res_settings.rows or res_settings.rows[0][0] == '0': return None
        res = conn.execute("SELECT text FROM compliments WHERE type=? ORDER BY RANDOM() LIMIT 1", (c_type,))
