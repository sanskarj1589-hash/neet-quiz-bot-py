...
def init_db():
    with get_db() as conn:
        ...
        defaults = [('footer_text', 'NEETIQBot'), ('footer_enabled', '1'), ('autoquiz_enabled', '0'), ('autoquiz_interval', '30'), ('compliments_enabled', '1')]
...
def update_user_stats(user_id, chat_id, is_correct, username=None, first_name=None):
    ...
    with get_db() as conn:
        if first_name or username:
            conn.execute("INSERT OR REPLACE INTO users (user_id, username, first_name, joined_at) VALUES (?, ?, ?, COALESCE((SELECT joined_at FROM users WHERE user_id=?), ?))", (user_id, username, first_name, user_id, today))
        ...
        conn.execute("UPDATE stats SET attempted=attempted+1, correct=correct+?, score=score+?, current_streak=?, max_streak=?, last_date=? WHERE user_id=?", (1 if is_correct else 0, points, new_streak, new_max, today, user_id))
        if chat_id < 0:
            conn.execute("INSERT OR IGNORE INTO group_stats (chat_id, user_id) VALUES (?,?)", (chat_id, user_id))
            conn.execute("UPDATE group_stats SET score=score+?, attempted=attempted+1, correct=correct+? WHERE chat_id=? AND user_id=?", (points, 1 if is_correct else 0, chat_id, user_id))

def get_leaderboard_data(chat_id=None, limit=25):
    with get_db() as conn:
        name_logic = "COALESCE(CASE WHEN u.username IS NOT NULL AND u.username != '' THEN '@' || u.username ELSE NULL END, u.first_name, 'Participant')"
        if chat_id:
            return conn.execute(f"SELECT {name_logic}, gs.attempted, gs.correct, gs.score FROM group_stats gs LEFT JOIN users u ON gs.user_id = u.user_id WHERE gs.chat_id = ? ORDER BY gs.score DESC LIMIT ?", (chat_id, limit)).rows
        return conn.execute(f"SELECT {name_logic}, s.attempted, s.correct, s.score FROM stats s LEFT JOIN users u ON s.user_id = u.user_id ORDER BY s.score DESC LIMIT ?", (limit,)).rows
